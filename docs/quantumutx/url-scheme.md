...
#### 替换所有现有资源：`quantumult-x:///update-configuration?remote-resource=url-encoded-json`
...
#### 保留所有现有资源并添加新资源：`quantumult-x:///add-resource?remote-resource=url-encoded-json`
...
#### 添加脚本任务仓库：`quantumult-x:///ui?module=gallery&type=task&action=add&content=url-encoded-json`
...
#### 添加图标集仓库：`quantumult-x:///ui?module=gallery&type=icon&action=add&content=url-encoded-json`
